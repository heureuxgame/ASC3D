...
class VegasActivity  : ComponentActivity() {

    private val vegasActivityViewModel by viewModels<VegasActivityViewModel>()
    private lateinit var buttonArray: Array<Button>  // Score buttons
    private val TAG = "VegasActivity"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tVscr01.setText(R.string.x_count)

        // Set Scoring format
        //val scoreArray1: IntArray = mainActivityViewModel.mScores.scoreArrayASA
        val scoreArray1: Array<String> = vegasActivityViewModel.mScores.scoreArrayVegas
        ...
        textViewTargetVal.text = vegasActivityViewModel.mScores.activeTarget.toString()

        textViewTotalVal.text = vegasActivityViewModel.mScores.totalScore.toString()
        setButtons(buttonArray, scoreArray1)

        activeTargetHighlight(vegasActivityViewModel.mScores.activeTarget)
    ...
    private fun startASA(){
        startActivity(Intent(this@VegasActivity,MainActivity::class.java))
    }
     ...
     */
    private fun setButtons(buttonArray: Array<Button>, idArray: Array<String>) {

        for (i in buttonArray.indices) {
            if (i < idArray.size) {
                // Set scoreArray value as text on button
                buttonArray[i].text = idArray[i]
    ...
    private fun setScoreTextViews(textViewArray: Array<TextView>) {
        ...
        for (i in 1 until vegasActivityViewModel.mScores.scores.size) {
    ...
    private fun activeTargetHighlight(target: Int) {
        ...
        if (vegasActivityViewModel.mScores.activeTarget <= vegasActivityViewModel.mScores.scores.size - 1) {
    ...
    fun addScore(score: String) {
        if (vegasActivityViewModel.mScores.activeTarget <= vegasActivityViewModel.mScores.scores.size - 1) {
            // add score to array
            vegasActivityViewModel.addScore(vegasActivityViewModel.mScores.activeTarget, score)
            // tally scores in array
            vegasActivityViewModel.mScores.totalScore = vegasActivityViewModel.totalScore()
            //textViewScoreArray[activeTarget].setText(score.toString())
            textViewScoreArray[vegasActivityViewModel.mScores.activeTarget].text = score
            //activeTarget += 1 //Increment target count
            vegasActivityViewModel.mScores.activeTarget += 1 //Increment target count
            setDisplayAfterScore()
            Log.d(TAG, "mainActivityViewModel.totalScore = " + vegasActivityViewModel.mScores.totalScore)
            Log.d(TAG, "mainActivityViewModel.totalScore() = " + vegasActivityViewModel.totalScore())
        } else {
    ...
    private fun clearScores() {
        ...
        builder.setMessage("Do you want to clear all scores?")
            ...
            .setPositiveButton("Yes") { dialog, which ->
                vegasActivityViewModel.clearAllScores()
                setInitialDisplay()
    ...
    private fun setDisplayAfterScore() {
        textViewTotalVal.text = vegasActivityViewModel.mScores.totalScore.toString()
        Log.d(TAG, "1 setDisplayAfterScore activeTarget = " + vegasActivityViewModel.mScores.activeTarget)
        // get X count from array and update ui
        tVscr0.text = vegasActivityViewModel.xCount().toString()
        // Avoid getting index out of bounds
        if (vegasActivityViewModel.mScores.activeTarget > vegasActivityViewModel.mScores.scores.size - 1){
            Toast.makeText(this, "30 targets scored", Toast.LENGTH_SHORT).show()
            vegasActivityViewModel.mScores.activeTarget = vegasActivityViewModel.mScores.scores.size - 1
        }
        if (vegasActivityViewModel.mScores.activeTarget <= vegasActivityViewModel.mScores.scores.size - 1) {
            Log.d(
                TAG,
                "2 setDisplayAfterScore mainActivityViewModel.activeTarget = " + vegasActivityViewModel.mScores.activeTarget
            )
    ...
    private fun setInitialDisplay() {
        ...
        if (vegasActivityViewModel.mScores.activeTarget <= vegasActivityViewModel.mScores.scores.size - 1) {
            ...
            for (i in 1 until vegasActivityViewModel.mScores.scores.size) {
                var text = ""
                if (vegasActivityViewModel.mScores.scores[i] != null) {
                    text = vegasActivityViewModel.mScores.scores[i].toString()
                }
                textViewScoreArray[i].text = text
            }
            textViewScoreArray[vegasActivityViewModel.mScores.activeTarget].setBackgroundColor(
                ContextCompat.getColor(applicationContext, R.color.highlight)
            )

            textViewTotalVal.text = vegasActivityViewModel.mScores.totalScore.toString()
            Log.d(TAG, "1 setInitialDisplay activeTarget = " + vegasActivityViewModel.mScores.activeTarget)
            // get 12 count from array and update ui
            tVscr0.text = vegasActivityViewModel.mScores.xCount().toString()
            // Avoid getting index out of bounds
            if (vegasActivityViewModel.mScores.activeTarget <= vegasActivityViewModel.mScores.scores.size - 1) {
                Log.d(
                    TAG,
                    "2 setInitialDisplay mainActivityViewModel.activeTarget = " + vegasActivityViewModel.mScores.activeTarget
                )
                textViewTargetVal.text = vegasActivityViewModel.mScores.activeTarget.toString()
                activeTargetHighlight(vegasActivityViewModel.mScores.activeTarget)
            }
    ...
    private fun textViewOnClick(position: Int) {
        ...
        vegasActivityViewModel.mScores.activeTarget = position
